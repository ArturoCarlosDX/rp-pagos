...
const Dashboard = () => {
  ...
  const handleCreatePayment = async () => {
    ...
    try {
      const payload = {
        amount: 1250,
        description: "Pago demo desde RP Pagos",
        external_reference: `DEMO-${Date.now()}`,
      };
      const resp: any = await apiFetch("/payments/create", {
        method: "POST",
        body: JSON.stringify(payload),
      });
      // resp.checkout_url expected
      if (resp && (resp.checkout_url || resp.init_point)) {
        const url = resp.checkout_url || resp.init_point;
      ...
      toast({
        title: "Error",
        description: `${e?.message || "No se pudo crear el pago"}`,
      });
      setIsPaying(false);
    }
  };
        ...
        <div className="container mx-auto max-w-7xl space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {stats.map((stat, index) => (
              <Card
                key={index}
                className="glass-card hover:cosmic-glow transition-all"
              >
